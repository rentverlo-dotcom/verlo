...
function normalizeTimingBucket(
  value: string | null
): TimingBucket | null {
...
function isRoomsCompatible(
  tenantRooms: string | null,
  ownerRooms: string | null
) {
  if (
    !tenantRooms ||
    !ownerRooms
...
function isIncomeProofCompatible(
  ...
  return acceptedProofs.includes(
    tenantProof
  )
}
...
function isGuaranteeCompatible(
  ...
  if (
    !tenantGuarantees ||
    tenantGuarantees.length ===
      0 ||
    !acceptedGuarantees ||
    acceptedGuarantees.length ===
      0
  ) {
    return null
  }
  ...
  return realTenantGuarantees.some(
    (guarantee) =>
      acceptedGuarantees.includes(
        guarantee
      )
...
async function insertLeadNeighborhoods({
  ...
  const {
    ...
    await supabaseAdmin
      ...
      .upsert(
        ...
        {
          onConflict:
            "lead_id,context,neighborhood_slug",
  ...
  if (error) {
    console.error(
      "lead neighborhoods insert error:",
      error
    )
...
async function getNeighborhoodCompatibilityMap({
  ...
  if (error) {
    console.error(
      "neighborhood compatibility fetch error:",
      error
    )
  ...
  for (
    ...
    const compatible =
      String(
        row.compatible_neighborhood_slug
      )
...
async function createLeadMatches({
          ...
          (
            ...
            return {
              ...
              reasons: {
                ...
                neighborhood_match_type:
                  neighborhoodMatch.type,
                ...
                tenant_guarantee_types:
                  lead.guarantee_types,

                owner_accepted_income_proof_types:
                  ownerLead.accepted_income_proof_types,

                owner_min_income_ratio:
                  ownerLead.min_income_ratio,
  ...
  if (isOwner) {
    ...
    if (error) {
      console.error(
        "lead match tenant search error:",
        error
      )
...
export async function POST(
  ...
  try {
    ...
    const income_proof_type =
      clean(
        body.verlo_income_proof_type ||
          body.income_proof_type
      ) ||
      null

    const income_range =
      clean(
        body.verlo_income_range ||
          body.income_range
      ) ||
      null
    ...
    const guarantee_types =
      toStringArray(
        body.verlo_guarantee_types ||
          body.guarantee_types
      )
    ...
    const contract_expiration =
      clean(
        body.contract_expiration
      ) ||
      null

    const other_party_status =
      clean(
        body.other_party_status
      ) ||
      null
    ...
    const neighborhood_labels =
      ...
      0
        ...
        : toStringArray(
          ...
          : toStringArray(
              metadata.tenant_neighborhoods
            )

    const neighborhood_slugs =
      toStringArray(
        body.neighborhood_slugs
      ).length >
      0
        ? toStringArray(
            body.neighborhood_slugs
          )
        : neighborhood_labels.map(
            normalizeText
          )

    const neighborhood_slug =
      clean(
        body.neighborhood_slug ||
          metadata.neighborhood_slug
      ) ||
      (
        role ===
          "owner" &&
        neighborhood_labels.length >
          0
          ? normalizeText(
              neighborhood_labels[
                0
        ...
        return NextResponse.json(
          {
            ...
            error:
              "Elegí al menos un barrio donde buscarías alquilar",

            raw_neighborhood_labels:
              body.neighborhood_labels,

            raw_neighborhood_slugs:
              body.neighborhood_slugs,

            parsed_neighborhood_labels:
              neighborhood_labels,

            parsed_neighborhood_slugs:
              neighborhood_slugs,
          },
          {
            status: 400,
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Elegí el tipo de propiedad que buscás",
          },
          {
            status: 400,
          }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Elegí los ambientes que buscás",
          },
          {
            status: 400,
          }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Ingresá tu presupuesto mensual máximo",
          },
          {
            status: 400,
          }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Elegí el barrio de la propiedad",
          },
          {
            status: 400,
          }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Elegí los ambientes de la propiedad",
          },
          {
            status: 400,
          }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Ingresá el precio mensual esperado",
          },
          {
            status: 400,
          }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Elegí la disponibilidad de la propiedad",
          },
          {
            status: 400,
          }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Elegí si sos propietario o inquilino",
          },
          {
            status: 400,
          }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Ingresá la fecha de vencimiento del contrato",
          },
          {
            status: 400,
          }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Indicá si ya lo hablaste con la otra parte",
          },
          {
            status: 400,
          }
        ...
        return NextResponse.json(
          {
            ok: false,
            error:
              "Elegí qué querés lograr con la renovación",
          },
          {
            status: 400,
          }
    ...
    if (
      !neighborhoodResult.ok
    ) {
      console.error(
        "lead neighborhoods error:",
        neighborhoodResult
      )
    }

    const matchResult =
      await createLeadMatches({
        supabaseAdmin,
