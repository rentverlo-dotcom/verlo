...
export async function GET() {
  return NextResponse.json({ ok: true, endpoint: "intake-ghl", method: "GET" })
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isValidPhone(phone)) {
      return NextResponse.json({ ok: false, error: "Ingresá un WhatsApp válido" }, { status: 400 })
    }
    ...
    if (!["owner_new_listing", "tenant_search", "contract_renewal"].includes(intent)) {
      return NextResponse.json({ ok: false, error: "Intención inválida" }, { status: 400 })
    }

    const tags = getLeadTags({ role, intent, availability_status, move_timing, renewal_role })
    ...
    if (error) {
      console.error("intake-ghl insert error:", error)
      return NextResponse.json(
    ...
    if (!ghl.ok) console.error("ghl webhook error:", ghl.error)

    return NextResponse.json({ ok: true, endpoint: "intake-ghl", ghl })
  } catch (err) {
    console.error("intake-ghl api error:", err)
