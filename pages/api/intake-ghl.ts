...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (full_name.length < 3) {
      return res.status(400).json({ ok: false, error: "Ingresá tu nombre" })
    }
    ...
    if (!isValidPhone(phone)) {
      return res.status(400).json({ ok: false, error: "Ingresá un WhatsApp válido" })
    }
    ...
    if (!["owner_new_listing", "tenant_search", "contract_renewal"].includes(intent)) {
      return res.status(400).json({ ok: false, error: "Intención inválida" })
    }
    ...
    if (error) {
      console.error("pages api intake-ghl insert error:", error)
      return res.status(500).json({ ok: false, error: "No pudimos guardar tus datos" })
    }
    ...
    console.error("pages api intake-ghl api error:", err)
